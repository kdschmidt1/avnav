...
let read_parameter= function(parameter)
{
let self=parameter;
...
color=((props.LLBB-props.TWD)+540)%360-180 > 0 ? "rgb(0,255,0)":"red";
	...
	ctx.restore();
	ctx.save();		
	boatPosition = this.lonlat_to_Canvas([props.boatposition.lon,props.boatposition.lat]);
	...
	if(this.MapLayline)
		DrawMapLaylines(this, ctx, radius, this.parameter, props); 
	...
	try{
		...
		if (props.WPposition) {
			...
			if(is_SB_boat && is_SB_WP && is_BB_boat && is_BB_WP){	
				...
				self.MapLayline = null; // keine schnittpunkte
			}
		}
		catch (e) {
		...
		let reduktionszeit = self.parameter.Laylinerefresh * 60
...
let DrawMapLaylines=function(self,ctx, radius, parameter, props) {
	...
	if(parameter.LaylineBoat=='True')
	{
		// Layline vom Boot:
		// BBis
		p1=latloncoordinatetodevice([self.MapLayline.Boat.BB.P1._lon,self.MapLayline.Boat.BB.P1._lat]);
		p2=latloncoordinatetodevice([self.MapLayline.Boat.BB.P2._lon,self.MapLayline.Boat.BB.P2._lat]);
		DrawLine(p1,p2,((props.LLBB-props.TWD)+540)%360-180 < 0 ? "rgb(0,255,0)":"red");
		// SB
		p1=latloncoordinatetodevice([self.MapLayline.Boat.SB.P1._lon,self.MapLayline.Boat.SB.P1._lat]);
		p2=latloncoordinatetodevice([self.MapLayline.Boat.SB.P2._lon,self.MapLayline.Boat.SB.P2._lat]);
	...
	if(parameter.LaylineWP=='True')
	{
		...
		// BB
		p1=latloncoordinatetodevice([self.MapLayline.WP.BB.P1._lon,self.MapLayline.WP.BB.P1._lat]);
		p2=latloncoordinatetodevice([self.MapLayline.WP.BB.P2._lon,self.MapLayline.WP.BB.P2._lat]);
		DrawLine(p1,p2,((props.LLBB-props.TWD)+540)%360-180 > 0  ? "rgb(0,255,0)":"red");
		// SB
		p1=latloncoordinatetodevice([self.MapLayline.WP.SB.P1._lon,self.MapLayline.WP.SB.P1._lat]);
		p2=latloncoordinatetodevice([self.MapLayline.WP.SB.P2._lon,self.MapLayline.WP.SB.P2._lat]);
		DrawLine(p1,p2,((props.LLSB-props.TWD)+540)%360-180 > 0  ? "rgb(0,255,0)":"red");
